...
AssistantResponse QueryProcessor::handleContinuationAfterConfirmation(MessageBuilder& messageBuilder) {
    ...
    if (last_message.value("role", "") == "assistant" && last_message.contains("tool_calls")) {
        ...
        return handleToolCalls(last_message, messageBuilder);
    }
    // Should not happen, but as a fallback, indicate success to continue to ReAct loop
    return {.is_final = false};
}

AssistantResponse QueryProcessor::handleToolCalls(const nlohmann::json& message, MessageBuilder& messageBuilder) {
    const auto& tool_calls = message["tool_calls"];
    for (const auto& call : tool_calls) {
        auto tool_response = executeToolCall(call, messageBuilder);
...
AssistantResponse QueryProcessor::executeToolCall(const nlohmann::json& call, MessageBuilder& messageBuilder) {
